...
async function checkConnection({ announce = false } = {}) { refresh.disabled = true; testConnection.disabled = true; connectionStatus.textContent = "Checking connection"; connectionDot.classList.remove("is-connected", "is-offline"); try { const response = await chrome.runtime.sendMessage({ type: "fluxdm:ping" }); const connected = setConnection(response); if (announce) setMessage(connected ? "FluxDM is ready for downloads." : "Open FluxDM, then try again.", !connected) } catch { setConnection(null); if (announce) setMessage("Open FluxDM, then try again.", true) } finally { refresh.disabled = false; testConnection.disabled = false } }
