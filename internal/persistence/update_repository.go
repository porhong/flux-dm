...
func (r *UpdateRepository) Load(ctx context.Context) (update.StoredState, error) {
	...
	err := r.db.QueryRowContext(ctx, `SELECT channel,auto_download,phase,available_version,release_notes_url,installer_path,installer_sha256,downloaded_bytes,total_bytes,last_checked_at,last_error FROM update_state WHERE id=1`).Scan(
		...
		&state.LastCheckedAt, &state.LastError,
	)
	if err != nil {
...
func (r *UpdateRepository) Save(ctx context.Context, state update.StoredState) error {
	...
	_, err := r.db.ExecContext(ctx, `UPDATE update_state SET channel=?,auto_download=?,phase=?,available_version=?,release_notes_url=?,installer_path=?,installer_sha256=?,downloaded_bytes=?,total_bytes=?,last_checked_at=?,last_error=?,updated_at=CURRENT_TIMESTAMP WHERE id=1`,
		state.Channel, autoDownload, state.Phase, state.AvailableVersion, state.ReleaseNotesURL,
		state.InstallerPath, state.InstallerSHA256, state.DownloadedBytes, state.TotalBytes,
		state.LastCheckedAt, state.LastError,
	)
