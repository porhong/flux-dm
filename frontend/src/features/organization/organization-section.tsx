...
function CategoryPanel({ categories, onChanged }: { categories: Category[]; onChanged: () => Promise<void> }) {
  ...
  return <Panel title="File categories" description="Extension rules choose a category and optional destination." icon={FolderCog}>
    <form className="grid gap-3 border-b border-border p-4 md:grid-cols-[1fr_1.3fr_1.5fr_6rem_auto]" onSubmit={(event)=>void submit(event)}>
...
function QueuePanel({ queues, onChanged }: { queues: DownloadQueue[]; onChanged: () => Promise<void> }) {
  ...
  return <Panel title="Download queues" description="Queue policy caps parallel work, connections, and bandwidth." icon={ListOrdered}>
    <form className="grid gap-3 border-b border-border p-4 md:grid-cols-[1.2fr_6rem_7rem_6rem_7rem_auto_auto]" onSubmit={(event)=>void submit(event)}>
      ...
      <select aria-label="Max connections" className="ui-field rounded-md px-3 text-sm" value={connections} onChange={(e)=>setConnections(Number(e.target.value) as typeof connections)}>{connectionChoices.map((value)=><option key={value}>{value}</option>)}</select>
...
function Panel({title,description,icon:Icon,children}:{title:string;description:string;icon:typeof FolderCog;children:ReactNode}) { return <section className="ui-panel overflow-hidden rounded-2xl"><header className="ui-panel-header flex items-center gap-3 px-5 py-4"><Icon className="size-5 text-cyan-300"/><div><h2 className="font-medium">{title}</h2><p className="text-xs text-slate-500">{description}</p></div></header>{children}</section> }
