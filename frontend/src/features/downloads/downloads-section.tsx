...
const DownloadRow = memo(function DownloadRow({ item, selected, onToggle, onProperties, onFileAction }: { item: DownloadItem; selected: boolean; onToggle: (id: string) => void; onProperties: (item: DownloadItem) => void; onFileAction: (kind: "open" | "reveal" | "rename" | "move" | "delete", item: DownloadItem) => void }) {
  ...
  return <div role="row" tabIndex={0} aria-selected={selected} onKeyDown={onKeyDown} onDoubleClick={() => onProperties(item)} className={`download-list-row ${isCompleted ? "download-list-row--completed" : ""} grid h-full items-center border-b border-white/6 px-4 outline-none focus:bg-cyan-400/[0.05] ${selected ? "bg-cyan-400/[0.04]" : "hover:bg-white/[0.02]"}`}>
    ...
    <div role="cell" className="min-w-0"><Badge variant={stateVariant(item.state)}>{stateLabel(item.state)}</Badge>{item.lastError && <div className="mt-1 break-words text-xs leading-4 text-red-300" title={`${item.lastError}\n${downloadFailureGuidance(item.lastError)}`}>{compactDownloadError(item.lastError)}</div>}{actionError && <div className="mt-1 text-xs text-red-300">{actionError}</div>}</div>
...
function CompletedFileDialog({ dialog, onOpenChange, onCompleted, onError }: { dialog: FileDialog | null; onOpenChange: (open: boolean) => void; onCompleted: (result: CompletedFileOperationResult) => void; onError: (message: string) => void }) {
  ...
  const [deleteMode, setDeleteMode] = useState<"history" | "recycle" | "permanent">("recycle")
  ...
  const submit = async () => {
    ...
    try {
      if (dialog.kind === "rename") {
        ...
        onCompleted(deleteMode === "recycle" ? await recycleCompletedDownloadFiles(ids) : deleteMode === "permanent" ? await deleteCompletedDownloadFiles(ids) : await removeCompletedDownloadHistory(ids))
      } else {
  ...
  return <Dialog open onOpenChange={onOpenChange}><DialogContent><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>{dialog.kind === "rename" ? "A conflicting name will receive a unique numeric suffix." : dialog.kind === "move" ? `Choose where to move ${countLabel}. Existing files will not be overwritten.` : dialog.kind === "delete" ? `Choose how to remove ${countLabel}.` : `${countLabel} can run code. FluxDM will only open it because you explicitly confirm this action.`}</DialogDescription></DialogHeader>{dialog.kind === "rename" && <label className="block space-y-2 text-sm"><span>Filename</span><Input aria-label="New filename" value={fileName} onChange={(event) => setFileName(event.target.value)} /></label>}{dialog.kind === "move" && <label className="block space-y-2 text-sm"><span>Destination folder</span><div className="flex gap-2"><Input aria-label="Move destination folder" value={directory} onChange={(event) => setDirectory(event.target.value)} /><Button type="button" variant="outline" onClick={() => void browse()}><FolderOpen className="size-4" /> Browse</Button></div></label>}{dialog.kind === "delete" && <div className="space-y-3 text-sm"><label className="flex items-start gap-2"><input aria-label="Move files to Recycle Bin" type="radio" checked={deleteMode === "recycle"} onChange={() => setDeleteMode("recycle")} /><span><strong>Move files to Recycle Bin and remove history</strong><br /><span className="text-slate-400">Files can be restored from the Recycle Bin outside FluxDM.</span></span></label><label className="flex items-start gap-2"><input aria-label="Permanently delete files" type="radio" checked={deleteMode === "permanent"} onChange={() => setDeleteMode("permanent")} /><span><strong>Permanently delete files and remove history</strong><br /><span className="text-slate-400">This cannot be undone and also works when no Recycle Bin is available, including portable locations.</span></span></label><label className="flex items-start gap-2"><input aria-label="Remove history only" type="radio" checked={deleteMode === "history"} onChange={() => setDeleteMode("history")} /><span><strong>Remove history only</strong><br /><span className="text-slate-400">Keep files in their current locations.</span></span></label></div>}<div className="flex justify-end gap-2"><Button type="button" variant="ghost" onClick={close}>Cancel</Button><Button type="button" variant={dialog.kind === "delete" ? "destructive" : "default"} disabled={submitting || (dialog.kind === "rename" && !fileName.trim()) || (dialog.kind === "move" && !directory.trim())} onClick={() => void submit()}>{submitting && <LoaderCircle className="size-4 animate-spin" />}{dialog.kind === "openExecutable" ? "Open file" : dialog.kind === "delete" ? deleteMode === "recycle" ? "Recycle files" : deleteMode === "permanent" ? "Delete files permanently" : "Remove history" : dialog.kind === "move" ? "Move files" : "Rename file"}</Button></div></DialogContent></Dialog>
}
...
function compactDownloadError(message: string): string { if (/HTTP 401\b/.test(message)) return "HTTP 401 — authentication required"; if (/HTTP 403\b/.test(message)) return "HTTP 403 — access denied"; return message }
function downloadFailureGuidance(message: string): string { if (/HTTP 401\b/.test(message)) return "Authentication is required. Add authorized credentials in Settings → Site profiles, or use a direct link you are allowed to access."; if (/HTTP 403\b/.test(message)) return "The server refused this request. Verify this is a direct downloadable file. For a site you are authorized to use, start it from the browser integration with cookie sharing enabled."; return "Check the link, connection, and destination, then retry." }
