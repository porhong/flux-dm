...
$defaultApplicationRoot = if (Test-Path -LiteralPath (Join-Path $root 'FluxDM.exe')) { $root } else { Join-Path $root 'build\bin' }
...
if (-not (Test-Path -LiteralPath $desktopPath) -or -not (Test-Path -LiteralPath $nativeHostPath)) {
  throw "ApplicationRoot must contain FluxDM.exe and FluxDM.NativeHost.exe: $applicationDirectory"
}
