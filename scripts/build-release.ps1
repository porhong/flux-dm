...
try{
  ...
  wails build -clean -trimpath -nocolour -ldflags $buildLDFlags;if($LASTEXITCODE){throw 'Wails build failed'}
  ...
  wails build -nsis -s -skipbindings -trimpath -nocolour -ldflags $buildLDFlags -webview2 download;if($LASTEXITCODE -or -not(Test-Path -LiteralPath $installerPath)){throw 'NSIS build failed or produced no installer'}
